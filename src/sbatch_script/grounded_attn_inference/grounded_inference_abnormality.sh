srun -p medai --quotatype auto --job-name r8_inference_grounded_abnormality --gpus-per-task=1 --nodes=1 --ntasks=1 --cpus-per-task 24 \
    --chdir \
    --output \
    --error \
    torchrun --nproc_per_node=1 --master_port 18233 global_inference.py \
        ...
        --logging_steps 1 \
        --bf16 True \
        --run_name r8_inference_grounded_abnormality \
